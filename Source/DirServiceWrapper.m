...
@implementation DirServiceWrapper
...
- (NSDictionary*) attributeDataForNodeOfType: (const char*) type value: (const char*) value attr1: (const char*) attr1 attr2: (const char*) attr2 attr3: (const char*) attr3 attr4: (const char*) attr4 attr5: (const char*) attr5 attr6: (const char*) attr6 attr7: (const char*) attr7 attr8: (const char*) attr8 attr9: (const char*) attr9 attr10: (const char*) attr10 attr11: (const char*) attr11 {
	...
	pAttribsToGet = dsBuildListFromStrings( dsRef, attr1, attr2, attr3, attr4, attr5, attr6, attr7, attr8, attr9, attr10, attr11);
		...
		if (dsStatus == eDSNoErr)
		{
			...
			for (ii = 1 ; ii <= dsRecordEntryPtr->fRecordAttributeCount; ii++)
			{
				...
				if( dsStatus == eDSNoErr ) {
					...
					} else if (strcmp(dsAttributeEntryPtr->fAttributeSignature.fBufferData, kDSNAttrGroupMembership) == 0) {
						printf("HURRRRRRRRRRAAAAAAAAAAAAAAA!: %s",  dsAttributeValueEntryPtr->fAttributeValueData.fBufferData);
					}
				}*/
...
- (BOOL) authenticateUser: (const char*) username password: (const char*) password {
	NSDictionary* userInfo = [self attributeDataForNodeOfType: kDSStdRecordTypeUsers
								 					    value: username
														attr1: kDSNAttrRecordName
														attr2: kDSNAttrMetaNodeLocation
														attr3: NULL
														attr4: NULL
														attr5: NULL
														attr6: NULL
